package stevesvehicles.common.transfer;

import net.minecraft.inventory.Container;
import net.minecraft.inventory.IInventory;
import net.minecraft.inventory.Slot;
import net.minecraft.item.ItemStack;
import stevesvehicles.common.container.slots.ISpecialItemTransferValidator;
import stevesvehicles.common.container.slots.ISpecialSlotValidator;
...
public class TransferHandler {
	...
	public static void TransferItem(ItemStack iStack, IInventory inv, int start, int end, Container cont, java.lang.Class validSlot, java.lang.Class invalidSlot, int maxItems, TransferType type, boolean fake) {
		...
		int pos;
		// this loop will loop as long as there's more items to add to the
		// inventory and it's still possible to add those
		do {
			// pos = -1 means there's no valid pos(we have to start at that)
			pos = -1;
			// first loop through the inventory once, this will try to find
			// already used stacks where we could put the items
			for (int i = startEmpty; i <= end; i++) {
				if (isSlotOfType(cont.getSlot(i), validSlot) && (invalidSlot == null || !isSlotOfType(cont.getSlot(i), invalidSlot))) {
					// a super heavy if-statement. This will just compare the
			...
			// if there wasn't any occupied slot with room for our items it's
			// time to search for an empty slot instead
			if (pos == -1) {
			...
			if (pos != -1) {
				...
				// if there's no items to be moved there won't be any difference
				// and therefore we have to exit(no infinite loops here)
				if (stackSize <= 0) {
