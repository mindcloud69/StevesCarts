	...
	public void drawGuiBackground(float f, int x, int y) {
		...
		if (useTabs()) {
			for (int i = 0; i < TAB_COUNT; i++) {
				...
				drawTexturedModalRect(target[0] + (TAB_WIDTH - TAB_CONTENT_WIDTH) / 2, target[1] + (TAB_HEIGHT - TAB_CONTENT_HEIGHT) / 2, TAB_CONTENT_SRC_X + (TEXTURE_SPACING + TAB_CONTENT_WIDTH) * i, TAB_CONTENT_SRC_Y, TAB_CONTENT_WIDTH,
						TAB_CONTENT_HEIGHT);
			}
		}
