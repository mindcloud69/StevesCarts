...
import stevesvehicles.client.gui.detector.DropDownMenu;
import stevesvehicles.client.gui.detector.DropDownMenuFlow;
...
public class GuiDetector extends GuiBase {
	...
	private boolean dropOnObject(int x, int y, LogicObject object, LogicObject drop) throws IOException {
		if (inRect(x, y, object.getRect())) {
			 ...
			 * drop.isChildValid(object)) { if (object.getParent() != null &&
			 * object.getParent().isChildValid(drop)) { LogicObject parent =
			 * object.getParent(); object.setParentAndUpdate(detector, null);
