...
import stevesvehicles.client.rendering.RenderVehicle;
...
public class ModelFrontTank extends ModelVehicle {
	private static final ResourceLocation TEXTURE = ResourceHelper.getResource("/models/tank_front.png");
	...
	public ModelFrontTank() {
		for (int i = 0; i < 2; i++) {
			...
			tankTopBot.addBox(-4, // X
					...
					1, // Size Z
					0.0F);
			tankTopBot.setRotationPoint(-14, // X
					3.5F - i * 7, // Y
					0 // Z
					);
			...
			tankFrontBack.addBox(-6, // X
					...
					1, // Size Z
					0.0F);
			tankFrontBack.setRotationPoint(-17.5F + i * 7, // X
					0, // Y
					0 // Z
					);
	...
	public void render(ModuleBase module, float yaw, float pitch, float roll, float multiplier, float partialTime) {
		...
		if (module != null) {
			...
			if (liquid != null) {
				RenderVehicle.renderLiquidCuboid(liquid, ((ModuleTank) module).getCapacity(), -14, 0, 0, 6, 6, 12, multiplier);
			}
		}
	}
}
