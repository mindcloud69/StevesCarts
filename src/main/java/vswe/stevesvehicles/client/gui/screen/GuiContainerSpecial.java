...
public abstract class GuiContainerSpecial /* extends GuiScreen */ extends GuiContainer {
	 ...
	 * touchscreenDropY; int currentX = touchscreenDropX + (int)(distanceX *
	 * returnPart); int currentY = touchscreenDropY + (int)(distanceY *
	 * returnPart); drawDraggedItem(touchscreenReturnItem, currentX, currentY,
	 * null, false); } GL11.glPopMatrix(); if (inventory.getItemStack() == null
	 * && selectedSlot != null && selectedSlot.getHasStack()) { ItemStack
	 * hoverItem = selectedSlot.getStack(); renderToolTip(hoverItem, x, y); }
	 ...
	 * fontRenderer = fontRendererObj; } itemRender.renderItemAndEffectIntoGUI(
	 * item, x, y); int textY = y - (useTextOffset ?
	 * TOUCHSCREEN_ITEM_TEXT_OFFSET_Y : 0);
	 * itemRender.renderItemOverlayIntoGUI(fontRendererObj, item, x, textY,
	 * info); zLevel = 0; itemRender.zLevel = 0; } protected void
	 * drawGuiContainerForegroundLayer(int mX, int mY) {} protected abstract
	 * void drawGuiContainerBackgroundLayer(float f, int mX, int mY); private
	 * void renderSlot(Slot slot, int x, int y) { ItemStack slotItem =
	 * slot.getStack(); boolean shouldSlotUnderlayBeRendered = false; boolean
	 * shouldSlotOverlayBeRendered = false; boolean shouldSlotBeRendered = slot
	 * != clickedSlot || touchscreenItem == null || isRightMouseClick; ItemStack
	 * movingItem = mc.thePlayer.inventory.getItemStack(); String info = null;
	 * if (slot == clickedSlot && touchscreenItem != null && isRightMouseClick
	 * && slotItem != null) { slotItem = slotItem.copy(); slotItem.stackSize /=
	 * 2; }else if (isItemBeingDragged && draggedItemSlots.contains(slot) &&
	 * movingItem != null) { if (draggedItemSlots.size() == 1) { return; } if
	 ...
	 * slotId = FAKE_SLOT_ID; }else if (slot != null){ slotId = slot.slotNumber;
	 * }else{ slotId = INVALID_SLOT_ID; } if (slotId != INVALID_SLOT_ID) { if
	 * (mc.gameSettings.touchscreen) { if (slot != null && slot.getHasStack()) {
	 * clickedSlot = slot; touchscreenItem = null; isRightMouseClick = button ==
	 * MOUSE_RIGHT_CLICK; }else { clickedSlot = null; } }else if
	 * (!isItemBeingDragged) { if (mc.thePlayer.inventory.getItemStack() ==
	 * null) { if (pickItem) { handleMouseClick(slot, slotId, button,
	 * ClickType.CLONE); }else { boolean isShiftClicking = slotId !=
	 * FAKE_SLOT_ID && isShiftKeyDown(); ClickType clickMode = ClickType.PICKUP;
	 * if (isShiftClicking){ doubleClickItem = slot.getHasStack() ?
	 * slot.getStack() : null; clickMode = ClickType.QUICK_MOVE; }else if
	 * (slotId == FAKE_SLOT_ID) { clickMode = CLICK_MODE_OUTSIDE; }
	 * handleMouseClick(slot, slotId, button, clickMode); } preventItemRelease =
	 * true; }else{ isItemBeingDragged = true; dragMouseButton = button;
	 * draggedItemSlots.clear(); if (button == MOUSE_LEFT_CLICK || button ==
	 * MOUSE_RIGHT_CLICK) { lastValidDragMouseButton = dragMouseButton; } } } }
	 * } doubleClickSlot = slot; doubleClickTime = time; doubleClickButton =
	 * button; }
	 ...
	 * draggedItemSlots.size() && Container.func_94527_a(slot, movingItem, true)
	 ...
	 * protected boolean checkHotbarKeys(int key) { if
