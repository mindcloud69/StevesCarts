...
public abstract class GuiManager extends GuiBase {
	...
	public void drawGuiForeground(int x, int y) {
		...
		for (int i = 0; i < 4; i++) {
			...
			drawMouseOver(
					LocalizationManager.CHANGE_TRANSFER_DIRECTION.translate() + "\n" + LocalizationManager.CURRENT_SETTING.translate() + ": " + (manager.toCart[i] ? LocalizationManager.TO_VEHICLE.translate() : LocalizationManager.FROM_VEHICLE.translate()),
					x, y, getArrowCoordinate(i));
			drawMouseOver(
	...
	protected enum ColorEffect {
		RED(237, 28, 36), BLUE(0, 114, 188), YELLOW(255, 242, 0), GREEN(57, 181, 74), BLACK(48, 48, 48), CLEAR(255, 255, 255);
		private float red;
		private float green;
		private float blue;
