...
public class ModuleFirework extends ModuleAttachment {
	...
	private ItemStack getCharge() {
		...
		for (int i = 0; i < getInventorySize(); i++) {
			...
			if (item != null) {
				...
				} else if (canHasModifier && !removedModifier && ((item.getItem() == Items.FIRE_CHARGE && modifierType == 1) || (item.getItem() == Items.GOLD_NUGGET && modifierType == 2) || (item.getItem() == Items.SKULL && modifierType == 3)
	...
	private int[] generateColors(int maxColorCount) {
		...
		ArrayList<Integer> colorPointers = new ArrayList<>();
		for (int i = 0; i < 16; i++) {
			if (maxColors[i] > 0) {
				colorPointers.add(i);
			}
		...
		for (int i = 0; i < colors.length; ++i) {
			colors[i] = ItemDye.DYE_COLORS[usedColors.get(i)];
		}
		for (int i = 0; i < getInventorySize(); i++) {
			ItemStack item = getStack(i);
			if (item != null) {
