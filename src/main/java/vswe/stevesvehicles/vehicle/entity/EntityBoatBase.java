...
public abstract class EntityBoatBase extends EntityBoat { // The only reason
	...
	protected void handleEntityInteraction() {
		/*if (!worldObj.isRemote) {
			...
			List list = worldObj.getEntitiesWithinAABB(EntityBoat.class, getEntityBoundingBox().expand(BOUNDING_BOX_EXPANSION, 0, BOUNDING_BOX_EXPANSION));
			if (list != null && !list.isEmpty()) {
				for (Object obj : list) {
					Entity entity = (Entity) obj;
					if (entity != riddenByEntity && entity.canBePushed() && entity != this) {
	...
	protected boolean handleBlockRemoval(Block block, BlockPos pos) {
